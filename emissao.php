...
/* //verifica data para busca de boletos, de 20 dias
	...
	*/
	//consulta banco
$consulta_b = mysqli_query("SELECT * FROM boletos WHERE status='1'") or die (mysqli_error());
$n_b 	    = mysqli_fetch_array($consulta_b);
$id_banco   = $n_b['id'];
//consulta fatura
$consulta_fatura = mysqli_query("SELECT id FROM faturas WHERE vencimento<='$data2' && vencimento>='$data' && status ='1' "); 

while($n_fatura = mysqli_fetch_array($consulta_fatura) ){
...
// Os valores abaixo podem ser colocados manualmente ou ajustados p/ formul�rio c/ POST, GET ou de BD (mysqli,Postgre,etc)	//

// CONSULTA FATURA

$consulta_boleto = mysqli_query("SELECT * FROM boletos WHERE id='$id_banco'");


$n_boleto = mysqli_fetch_array($consulta_boleto);
...
$consulta_fatura = mysqli_query("SELECT * FROM faturas WHERE id='$id'");


$n_fatura = mysqli_fetch_array($consulta_fatura);
...
$consulta_cliente = mysqli_query("SELECT * FROM clientes WHERE id='$id_cliente'");
